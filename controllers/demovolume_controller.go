...
import (
	...
	demov1 "k9s-operator-demo-volume/api/v1"
)
...
//+kubebuilder:rbac:groups=demo.volumes.nj.io,resources=demovolumes,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=demo.volumes.nj.io,resources=demovolumes/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=demo.volumes.nj.io,resources=demovolumes/finalizers,verbs=update
...
func (r *DemoVolumeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	l.Info("Entering Reconcile:", "req", req)

	volume := &demov1.DemoVolume{}
	r.Get(ctx, types.NamespacedName{Name: req.Name, Namespace: req.Namespace}, volume)

	l.Info("In Reconcile:Volume", "spec:", volume.Spec, "status", volume.Status)

	if volume.Spec.Name != volume.Status.Name {
		volume.Status.Name = volume.Spec.Name // update status with spec
...
func (r *DemoVolumeReconciler) reconcilePVC(ctx context.Context, volume *demov1.DemoVolume, l logr.Logger) error {
	...
	if err == nil {
		l.Info("In ReconcilePVC:", "Status", "PVC Found")
		return nil
	}
	...
	l.Info("In ReconcilePVC:", "Status", "Creating PVC")
